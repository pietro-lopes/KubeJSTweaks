...
ServerEvents.recipes(event => {
  ...
  const itemList = Item.list.filter(item => !["minecraft:barrier", "minecraft:air", "ae2:facade", "immersiveengineering:graphite_electrode"].includes(item.id) && item.getMaxStackSize() > 1)
...
//      console.log(`aa.laser("${ri()}", "${rit()}", ${rRange(60,20000)})`)
  ...
  KJSTweaks.runIfModPresent("ae2", () => {
    ...
    ae2.inscriber("oak_trapdoor",{bottom: "minecraft:emerald", top: "minecraft:grass_block", middle: "minecraft:oak_log"}) // will be a inscribe

    ae2.inscriber("oak_door", {bottom: "minecraft:diamond", top: "minecraft:dirt", middle: "minecraft:stick"}).mode("inscribe")

    ae2.inscriber("apple", {bottom: "minecraft:dirt", middle: "minecraft:stick",top: "minecraft:acacia_boat"}).mode("press")

    // This is for kubejstweaks:codec only
    //ae2.inscriber("oak_door",["minecraft:diamond", "minecraft:dirt",  "minecraft:stick"]).mode("inscribe")

    event.replaceInput({type: "ae2:inscriber"}, Ingredient.of("#c:silicon"), Ingredient.of("minecraft:gold_block"))

    testRecipes(event, /.*/, "ae2:inscriber")
  })

  KJSTweaks.runIfModPresent("ars_nouveau", () => {
    let counter = 0
    runFor(5, (idx) => {
  ...
  KJSTweaks.runIfModPresent("farmingforblockheads", () => {
    let counter = 0
    runFor(5, (idx) => {
  ...
  KJSTweaks.runIfModPresent("functionalstorage", () => {
    let counter = 0
    runFor(10, (idx) => {
  ...
  KJSTweaks.runIfModPresent("immersiveengineering", () => {
    let counter = 0
    ...
    runFor(10, (idx) => {
      event.recipes.immersiveengineering.arc_furnace(
        [{basePredicate: riot(), count: rRange(2,10)}], // results
        rRange(20,400), // time
        rRange(10000,50000), // energy
        {basePredicate: riot(), count: rRange(2,10)} // input
      ).id("atmindev:immersiveengineering/arc_furnace/test/" + counter++)

      event.recipes.immersiveengineering.arc_furnace(
        [Item.of(ri()).copyWithCount(rRange(2,10)),{basePredicate: riot(), count: rRange(2,10)}], // results
        rRange(20,400), // time
        rRange(10000,50000), // energy
        riot(), // input
        {basePredicate: riot(), count: rRange(2,10)} // slag
      ).id("atmindev:immersiveengineering/arc_furnace/test/" + counter++)

      event.recipes.immersiveengineering.arc_furnace(
        [Item.of(ri()).copyWithCount(rRange(2,10)),{basePredicate: riot(), count: rRange(2,10)}, riot()], // results
        rRange(20,400), // time
        rRange(10000,50000), // energy
        riot(), // input
        riot(), // slag
        [{basePredicate: riot(), count: rRange(2,10)}, riot()], // additives
        [{output: riot(), chance: rChance()}] // secondaries
      ).id("atmindev:immersiveengineering/arc_furnace/test/" + counter++)

      event.recipes.immersiveengineering.arc_furnace(
        [riot(),riot(),riot(),riot()], // results
        rRange(20,400), // time
        rRange(10000,50000), // energy
        riot(), // input
        riot(), // slag
        [riot(),riot(),riot(),riot()], // additives
        [{output: riot(), chance: rChance()}], // secondaries
        riot() // specialRecipeType
      ).id("atmindev:immersiveengineering/arc_furnace/test/" + counter++)
    })
  ...
  KJSTweaks.runIfModPresent("industrialforegoing", () => {
    let counter = 0
    runFor(10, (idx) => {
    ...
    counter = 0
    runFor(10, (idx) => {
      event.recipes.industrialforegoing.fluid_extractor(rfs(), Block.getBlock(rBlock()).defaultBlockState(), riot(), rChance(), rb())
    ...
    testRecipes(event, /.*/, "industrialforegoing:laser_drill_ore")
  })

  KJSTweaks.runIfModPresent("integrateddynamics", () => {
    let counter = 0
    runFor(10, (idx) => {
  ...
  KJSTweaks.runIfModPresent("justdirethings", () => {

    let counter = 0
    runFor(10, (idx) => {
      event.recipes.justdirethings.fluiddrop(rFluidState(), "testing_" + counter, rFluidState(), ri())
